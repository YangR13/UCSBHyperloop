...
Maglev_BMS* initialize_Maglev_BMS(uint8_t identity) {
  ...
  for (batt = 0; batt < 3; batt++) {
    ...
    for (i = 0; i < 6; i++) {
      ...
      bms->cell_charge_coulomb[batt][i] = -1.0;
    }
...
uint8_t update_Maglev_BMS(Maglev_BMS* bms) {
  ...
  for (batt = 0; batt < 3; batt++) {
    ...
    for (i = 0; i < 6; i++){
    	...
    	bms->cell_charge_coulomb[batt][i] = voltageToCharge22(total_voltage) / 6.0; //charge for each cell
    	charge_sum += bms->cell_charge_coulomb[batt][i];
    }
    bms->charge_percent[batt] = percentCharge22(charge_sum);

    // Check cell voltages for a substantial imbalance, set an alarm if present
    float min = 9.9;
    float max = 0.0;
    for (i = 0; i < 5; i++){		//Should be 6 instead of 5?
...
void update_BMS_18V5(BMS_18V5* bms) {
  ...
  for (batt = 0; batt < 4; batt++) {
    ...
    for (i = 0; i < 5; i++){
        ...
        bms->cell_charge_coulomb[batt][i] = voltageToCharge22(total_voltage) / 5.0; //charge for each cell
        charge_sum += bms->cell_charge_coulomb[batt][i];
    }
    bms->charge_percent[batt] = percentCharge22(charge_sum);
